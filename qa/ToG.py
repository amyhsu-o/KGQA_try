...
"""
    ...
    def retrieve(self, query: str, root_path: Optional[str] = None) -> KG:
        ...
        # 1. initialization
        current_query = query
        query_topic_entities_score = self._get_topic_entities(query)
        ...
        while not data_sufficient and not early_stop:
            ...
            for topic_entity, topic_entity_score in topic_entities_score.items():
                ...
                relations_score = self._relation_prune(query, topic_entity, relations)
                ...
                for relation, relation_score in relations_score.items():
                    ...
                    entities_score = self._entity_prune(query, relation, entities)
            ...
            if not data_sufficient:
                ...
                if self.generate_new_query:
                    next_round_query = self._generate_query(query, used_kg)
                    topic_entities_score = self._get_topic_entities(next_round_query)
                ...
                if len(topic_entities_score) == 0:
                    topic_entities_score = self._get_topic_entities(query)
                    if (
                        sum(
                            [
